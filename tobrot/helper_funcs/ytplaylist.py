...
async def yt_playlist_downg(message, i_m_sefg, G_DRIVE):
    ...
    try:
        ...
        pass
    cmd = ["youtube-dl", "-i", "-f", "bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4", "-o", f"{fol_der}/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s", "--external-downloader", "aria2c", "--external-downloader-args, "-j 16 -x 16 -s 16 -k 5M", f"{url}"]
